...
describe('CalculatorFormComponent', () => {
  ...
  test('inputで受け取った値をフォームにセットする', () => {
    expect(spectator.component.form.value.skillDamage.skillDamage).toEqual(initialState.calculatorParams.skillDamage);
  });
  ...
  test('フォームが変更されたとき、新しい値をvalueChangeイベントで出力する', async () => {
    ...
    spectator.component.form.patchValue({ skillDamage: { skillDamage: 1000 } });
    spectator.detectChanges();

    expect(outputValue.skillDamage).toBe(1000);
  });
});
